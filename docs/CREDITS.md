...
**PokeBrawl** — 비상업 팬 프로젝트 (포켓몬 로그라이트)
...
- `bars.png`에서는 팔레트만 가져왔다 — 바 윤곽 `#494949`, 트랙 회색, 체력 초록 두 톤,
  경험치 푸른색 두 톤, HP 꼬리표 호박색

### 타이틀 로고

`Assets/Game/Art/UI/logo.png` — "Pokémon BRAWL" 로고. **출처를 확인하지 못했다.** 팀에서
받은 그림이고 그림 안에 서명이나 표기가 없다. "Pokémon" 글자는 원작 로고의 형태를 그대로
따르고 있으므로 상표는 닌텐도/크리처스/게임프리크의 것이다.

게임에 쓰는 `Resources/UI/PmdLogo.png`는 이 그림에서 **흰 배경과 테두리 얼룩을 걷어내고
여백을 잘라 낸 것**이다 (`scratchpad/bake_logo.py`). 모양과 색은 원본 그대로이고 새로 그린
부분은 없다.

### 타이틀 배경

`Resources/UI/TitleMaps/*.png` — 타이틀 뒤로 흐르는 21장은 **이 프로젝트의 방 프리팹을
에디터에서 그대로 찍은 것**이다 (`Assets/Game/Editor/TitleMapSetup.cs`). 그 안의 타일과
포켓몬 스프라이트의 출처는 위의 「환경 타일」·「캐릭터 스프라이트」와 같다.
