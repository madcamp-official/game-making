...
- 아이콘은 `Assets/Game/Art/Items/item.png`에서 잘라 쓴다. 41px 격자·40px 셀이고 좌상단이 (1, 1)이라 스프라이트 rect는 `x = 1 + 41*열`, `y = 1969 - 41*행`이다. 이 시트는 원래 `spriteMode: 0`(잘라 두지 않음)이라 슬라이스를 넣어도 스프라이트가 안 생겼다 — `spriteMode: 2`(Multiple)여야 한다
