...
public static class UiSpriteSetup
{
    ...
    private static readonly (string name, Vector4 border)[] Sprites =
    {
        ...
        // 꼬리표는 두 벌이다. 얇은 쪽은 기술 칸의 속성 표 — 칸 안에 들어앉는 작은 조각이라
        // 테두리가 두꺼우면 표가 글자보다 먼저 눈에 들어와 답답해진다. 굵은 쪽은 좌하단 두 바의
        // "HP"·"EXP" 표로, 바로 옆 바 틀과 두께가 같아야 표가 바에 눌려 보이지 않는다.
        // ⚠️ 굵은 쪽은 칸 높이도 함께 키워야 한다 — 안쪽 = 칸 높이 − 8이고 여기에 글자칸(24)이
        //    여유 있게 들어가야 하므로 36 이상이다 (BarFill.BarHeight).
        ("PmdChip", new Vector4(2, 2, 2, 2)),
        ("PmdChipBold", new Vector4(4, 4, 4, 4)),
