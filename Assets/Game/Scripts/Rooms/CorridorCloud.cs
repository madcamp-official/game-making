...
/// 엔딩 안개(fog.png)에서 덩이를 잘라 이어 붙였다. 예전에는 옆으로 흘러가는 프레임을
/// 돌렸는데, 좁은 통로를 꽉 채운 것이 움직이면 벽이 아니라 물살처럼 보여 뺐다 —
/// 가만히 차 있어야 "막혔다"로 읽힌다. 방을 향한 면이 둥글게 사그라드는 쪽이라,
/// 왼쪽 통로에는 그림을 뒤집어 쓴다.
///
/// <b>실행 중에 짙기를 건드리지 않는다.</b> 한때는 알파를 천천히 오르내리게 했지만(숨쉬기),
/// 통로를 꽉 채운 안개가 통째로 옅어졌다 짙어졌다 하는 것이 오히려 어색했다 — 안개는
/// 가만히 있어야 하고, 대신 <b>덩이마다 짙기가 달라야</b> 살아 보인다. 그 차이는 그림 한 장에
/// 구워 두었다(<c>scripts/bake_corridor_cloud.py</c>, 덩이별 알파 배율 0.90~1.00).
///
...
public class CorridorCloud : MonoBehaviour
{
    ...
    /// <param name="block">몸을 막을 자리(방 기준). 그림이 아니라 통로 구멍에 맞춘다.</param>
    public static CorridorCloud Create(Transform parent, string name, Vector2 localPosition,
                                       Vector2 size, Rect block, Sprite sprite, bool faceRight)
    {
        ...
        // ⚠️ 막는 자리는 그림이 아니라 <b>통로 구멍</b>에 맞춘다. 여기가 두 번 어긋났다 —
        // 처음에는 1x1이라 통로 한가운데 점만 막아 위아래로 비켜 갈 구멍이 났고, 그 반동으로
        // 그림 크기에 방 쪽으로 반 칸을 더 내밀었더니 이번에는 벽 안쪽 면보다 안으로 들어와
        // 방 안에 보이지 않는 턱이 생겼다. 지금은 RoomGates가 넘겨주는 네모 그대로 막는다.
        Vector3 s = go.transform.localScale;
        Vector2 offset = block.center - localPosition;
        box.offset = new Vector2(offset.x / s.x, offset.y / s.y);
        box.size = new Vector2(block.width / s.x, block.height / s.y);
